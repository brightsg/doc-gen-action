...
describe("resolveDocTypes", () => {
  it("should resolve api to shared + api", () => {
    ...
    expect(result).toEqual(["architecture", "onboarding", "release-notes", "changelog-internal", "changelog-external", "api"]);
  });

  it("should resolve worker to shared + worker", () => {
    const result = resolveDocTypes("worker");
    expect(result).toEqual(["architecture", "onboarding", "release-notes", "changelog-internal", "changelog-external", "worker"]);
  });

  it("should resolve sdk to shared + sdk-usage", () => {
    const result = resolveDocTypes("sdk");
    expect(result).toEqual(["architecture", "onboarding", "release-notes", "changelog-internal", "changelog-external", "sdk-usage"]);
  });

  it("should resolve frontend to shared + frontend-usage", () => {
    const result = resolveDocTypes("frontend");
    expect(result).toEqual(["architecture", "onboarding", "release-notes", "changelog-internal", "changelog-external", "frontend-usage"]);
  });

  it("should resolve cli to shared + cli-reference", () => {
    const result = resolveDocTypes("cli");
    expect(result).toEqual(["architecture", "onboarding", "release-notes", "changelog-internal", "changelog-external", "cli-reference"]);
  });

  it("should resolve comma-separated types and deduplicate shared docs", () => {
    const result = resolveDocTypes("api,worker");
    expect(result).toEqual(["architecture", "onboarding", "release-notes", "changelog-internal", "changelog-external", "api", "worker"]);
  });
